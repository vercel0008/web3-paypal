...
//Create Struct and Mappping for request, transaction & name
    ...
    struct userName {
        string name;
        bool hasName; // the default value is false
    }
...
//Create a Request to other user
...
//Pay a Request
    function payRequest(uint256 _request) public payable {
        require(_request < requests[msg.sender].length, "Request not available. No such request!");
        request[] storage myRequests = requests[msg.sender];
        request storage payableRequest = myRequests[_request];

        uint256 amountToPay = payableRequest.amount * 1000000000000000000;
        require(msg.value == amountToPay, "Amount incorrect!");
